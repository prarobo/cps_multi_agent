function [GTS] = loadGameMulti(states,trans,labels,Q0)
...
%initialize vars for sparse transition matrix
% ii = []; 
% jj = [];
% ss = [];
GTS.Q = states;
...
%     
% end
trans = double(cell2mat(cellfun(@cell2mat, trans, 'UniformOutput', false)));
ii = trans(:,1);
jj = trans(:,2);
ss = ones(size(trans,1),1);

GTS.adj = logical(sparse(ii,jj,ss,length(states),length(states))); %sparse transition matrix
